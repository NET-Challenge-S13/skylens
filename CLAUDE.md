...
| `res/docs/COMPONENTS.md` | **컴포넌트 구성의 단일 출처**. 8개 컴포넌트(드론·게이트웨이·프록시·코어·모델·클라이언트·shared·데모)의 역할과 망 위치, Gateway/WebRTC 두 데이터 흐름, 관제탑 화면 통합 규칙(§4), 데모 시나리오(§5), 디렉터리 구조(§6), 포트 맵(§7), 구현상의 결정(§8) |
...
| 컴포넌트 경계·서버·프로토콜 관련 작업 | **`res/docs/COMPONENTS.md` 필독** → `src/shared/protocol.ts` |
| 데모·뷰어(관제탑/현황판) UI·연출 작업 | **`res/docs/COMPONENTS.md`** → `PROJECT.md` → `README.md` |
...
컴포넌트 경계와 각 컴포넌트의 책임은 **`res/docs/COMPONENTS.md`가 단일 출처**다.
