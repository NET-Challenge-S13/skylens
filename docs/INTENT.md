...
재난 현장에서 사람과 위험구역을 **놓치지 않는 것**을 기준으로 정한다. 사람을 놓치면 되돌릴 수 없지만, 잘못 찍힌 마커는 지휘관이 한 번 더 확인하면 된다. 그래서 사람 검출은 재현율을 주 지표로 둔다.

판정에 쓰는 사람 지표 네 개는 **모두 점 기준**이다. 최종 산출물이 3D 좌표로 투영할 점 하나이기 때문이다. 중간보고서가 쓰던 박스 mAP@0.5 는 판정에서 빼되 **지표 자체는 지우지 않고 참고값으로 계속 보고한다.** 중간보고서와 수치를 비교할 수 있어야 하고, 선행 연구 비교도 박스 mAP 축에서 이뤄지기 때문이다.
...
| 사람 검출 | **점 AP**(거리 기준 average precision) | **0.65** | 없음 |
| 사람 검출 | 박스 mAP@0.5 (판정하지 않음, 참고) | 판정 없음 | 0.65 |
...
- **박스 mAP@0.5 는 판정 지표에서 뺐고, 그 자리에 같은 높이(0.65)의 점 AP 를 뒀다.** 박스 지표를 목표에 넣었던 이유는 중간보고서와의 연속성 하나였고, 이 과제가 요구하는 능력은 아니었다. 산출물이 점 하나인데 판정에만 상자가 남아 있으면, 상자 품질을 위해 공유 백본을 쓰는 대가로 재현율과 위험구역을 내주는 선택을 강요받는다. 그 대가가 실제로 존재한다는 것은 상자 회귀를 백본에서 끊어 확인했다(experiment/wh-detach #72: 상자 품질은 무너졌지만 SARD 재현율 0.8813 → 0.9012, 도로 차단 IoU 0.3249 → 0.4175). 지표의 종류만 바꾸고 바의 높이는 0.65 그대로 옮겼으며, 이 결정은 두 모델을 다시 재기 전에 내렸다. 근거: experiment/point-ap-primary (#73).
