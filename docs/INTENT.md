...
5. **사람 검출을 공유 백본의 stride 4 헤드로 목표(F1 0.70 · mAP@50 0.65)까지 올릴 수 있나.** 실제 추론 방식(LLVIP 전체 프레임, VisDrone 원본 스케일 타일)으로 합산했을 때 최고가 F1 0.643 · mAP@50 0.355 다. LLVIP 만으로는 mAP@50 0.695 로 목표를 넘지만 VisDrone(사람 폭 중앙값 약 8px)에서 상자 mAP 가 0.29 에 막힌다. 손실 가중치 · 손실 형태 · stride · 오프셋 · 오버샘플링 · 입력 해상도로는 풀리지 않았다(#30 #31 #35 #36 #37 #40 #41). 답에 따라 N3 의 "단일 백본 하나로" 가 사람 헤드에 한해 바뀔 수 있다.
...
- **사람 목표는 어떤 평가로 판정하나 → 실제 추론 방식.** LLVIP 는 전체 프레임을 512 로 줄여, VisDrone 은 짧은 변 765 로 맞춘 원본 스케일의 512 타일로 추론하고, 두 소스의 검출을 한 precision-recall 로 합산한다. 512 로 줄인 기존 평가는 비교용으로 함께 적는다.
