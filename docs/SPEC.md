...
- 헤드별로 분리 학습한다.
- **`road_blocked` 픽셀이 있는 학습 이미지는 오버샘플링한다.** 배수는 8이다. 이 클래스는 학습 표본의 약 2%에만 나타나 대부분의 배치에 아예 들어오지 않는다. 배수 4에서 IoU 0.0266 이 나왔고, 위험구역 평균 목표 0.55 에는 0.107 이 필요하다. 근거: experiment/road-oversample.
