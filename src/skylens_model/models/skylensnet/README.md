...
| 지표 | 대상 | 쓰는 이유 |
|---|---|---|
| `point_f1` | 사람 | 거리(8px) 매칭 P/R/F1. **파이프라인의 실제 목표** — 최종 산출물이 점이다(§1.4) |
| `point_ap` | 사람 | 같은 거리 매칭을 score 곡선 전체에 적용. `point_f1` 의 임계값 의존성을 제거 |
| `mAP@0.5` · `mAP@0.5:0.95` | 사람 | bbox 기준. **문헌 비교용** — 논문들이 이 값으로 보고한다 |
| `miou` · `iou_class_*` | 위험구역 | 클래스별 IoU. `miou` 는 배경이 끌어올리므로 **클래스별로 봐야 한다** |

`mAP@0.5` 와 `mAP@0.75` 의 격차는 "찾기는 하는데 박스 크기가 부정확"을 뜻한다.
`wh` 헤드는 depth 샘플링 보조용이라 loss 가중치가 0.1 이므로 이 격차는 정상이며,
우리 파이프라인에서 실질적 손해가 아니다(§1.4).

### 4.2 문헌 기준선

**직접 비교는 성립하지 않는다.** 아래 논문들은 데이터셋 하나에 전용 모델을 쓰지만,
우리는 4개 데이터셋을 섞어 단일 백본에 두 헤드를 얹고 512px 로 학습한다. 참고선이다.

| 과제 | 데이터셋 | 공개 결과 |
|---|---|---|
| 사람 탐지 | LLVIP | YOLOv5 AP50 **0.965**(적외선) / 0.908(가시광) |
| 사람 탐지 | VisDrone | pedestrian AP **0.323** (DPNet-ensemble) |
| 위험구역 세그 | RescueNet | PSPNet mIoU **0.794** (11클래스) |

LLVIP(0.97)과 VisDrone(0.32)의 격차가 크다는 점이 중요하다. 야간 열화상에서 사람은
밝게 도드라지지만 드론 고도의 보행자는 수십 픽셀에 밀집·가림이 심하다. 둘을 섞어
학습하므로 우리 목표선은 그 사이에 놓인다.

### 4.3 목표선

| 지표 | 최소 (데모 성립) | 양호 | 우수 |
|---|---|---|---|
| `mAP@0.5` (사람) | 0.50 | **0.65** | 0.75+ |
| `point_f1` | 0.60 | **0.70** | 0.80+ |
| `point_ap` | 0.50 | **0.62** | 0.70+ |
| `miou` (4클래스) | 0.45 | **0.55** | 0.65+ |
| `iou_class_2` 붕괴 | **0.30** | 0.45 | 0.60+ |
| `iou_class_1` 화재 | 0.35 | 0.50 | 0.65+ |

`miou` 목표를 0.55 로 둔 근거: RescueNet 의 0.794 는 11클래스 전용 모델에 고해상도
입력이다. 우리는 4클래스로 병합해 경계가 모호해졌고, 512px 로 줄였으며, 세그가
전체 배치의 일부만 차지한다. 0.79 의 70% 수준이 현실적 상한이다.

> **가장 중요한 기준은 `iou_class_2 > 0` 이다.** 다른 어떤 수치보다 우선한다.
> 0 이면 모델이 위험구역을 한 픽셀도 예측하지 않는다는 뜻이고, 그 상태의 `miou` 는
> 배경 클래스가 만든 허수다. 0.30 만 넘겨도 "AI 가 붕괴 지역을 찾는다"는 데모 서사가
> 처음으로 성립한다.

### 4.4 측정 이력

| 시점 | 학습셋 | mAP@0.5 | point_f1 | point_ap | miou | 화재 IoU | 붕괴 IoU | 도로차단 IoU |
|---|---|---|---|---|---|---|---|---|
| 1차 (5 ep) | 18,945장<br>세그 449장(2.4%) | **0.581** | **0.674** | **0.534** | 0.310 | nan(데이터 없음) | **0.000** | 0.000 |
| 2차 (5 ep) | 23,237장<br>세그 4,741장(20%) | 0.296 | 0.602 | 0.466 | **0.523** | **0.493** | **0.669** | 0.000 |

세그에 RescueNet train(3,595장)과 fire_seg(1,146장)를 넣어 세그 비중을 2.4% → 20% 로
올린 것이 유일한 변경점이다.

**1차 — 세그 완전 실패.** 혼동행렬상 GT 가 붕괴·도로차단인 픽셀의 **100%를 배경으로
예측**했고, 클래스별 평균 로짓이 `normal +2.00 / collapse -1.17 / road_blocked -1.68 /
fire -2.68` 로 배경만 양수였다. 데이터 부족에서 온 클래스 불균형 붕괴이며, 손실이
내려간 것은 상수 해로 수렴한 결과다. mIoU 0.310 은 배경 클래스가 만든 허수다.

**2차 — 세그는 목표 달성, 사람은 후퇴.** 붕괴 0 → 0.669(우수 구간), 화재는 첫 데이터
투입만으로 0.493 이 나왔다. **클래스 가중치 없이 데이터 증량만으로 불균형 붕괴가
해소**됐다. 반면 사람 지표는 전부 내려갔고 특히 mAP@0.5 가 0.581 → 0.296 으로 반토막
났다.

### 4.5 미해결 문제

**(1) 멀티태스크 간섭.** 에폭 1 에서 세그 IoU 가 전부 0 인 동안 mAP 도 0.124 로 낮았고,
에폭 2 에서 세그가 살아나는 순간(0 → 0.51) 사람 지표도 함께 올랐다. 두 헤드가 경쟁하는
것이 아니라 **공유 백본의 학습 예산을 나눠 쓴다.** 세그 비중이 8배가 되면서 사람 쪽
표현력이 희생됐다. loss 가중치(현재 `seg 1.0 / heatmap 1.0 / wh 0.1`)나 배치 샘플링
비율로 균형을 잡아야 한다.

**(2) 도로차단(클래스 3)은 두 차례 모두 정확히 0.** 붕괴·화재는 살아났는데 이것만
안 된다. RescueNet 에서 가장 희소하고 붕괴 잔해와 시각적으로 겹쳐 흡수된 것으로 보인다.

**(3) 5 epoch 는 부족하다.** 붕괴 IoU 가 0.511 → 0.614 → 0.658 → 0.669, mAP 가
0.234 → 0.296 으로 **끝까지 상승 중**이었다. 수렴하지 않았다.

**(4) 사람 헤드의 confidence 눌림(1차 진단, 2차에서도 유효).** score 0.5 를 넘는 예측이
하나도 없고, 임계값을 낮춰도 recall 이 0.66 에서 천장을 친다. CenterNet focal loss 에서
양성 픽셀이 극소수일 때 나타나는 현상이다.

> 노트북의 저장 후 `복원 일치: False` 는 **오탐이다.** `final` 과 `checkpoint-14525` 의
> 출력 차이는 정확히 `0.0` 이고 재로드도 정상이다. 검증 셀 쪽 문제이지 저장 문제가 아니다.
