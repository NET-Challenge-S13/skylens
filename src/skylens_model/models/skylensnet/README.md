...
| 시점 | 학습셋 | mAP@0.5 | point_f1 | point_ap | miou | 붕괴 IoU | 화재 IoU |
|---|---|---|---|---|---|---|---|
| 1차 (5 epoch) | 18,945장<br>세그 449장(2.4%) | 0.581 | 0.674 | 0.534 | 0.310 | **0.000** | nan(데이터 없음) |

1차 학습의 세그 실패 원인은 진단이 끝났다: 혼동행렬상 GT 가 붕괴·도로차단인 픽셀의
**100%를 배경으로 예측**했고, 클래스별 평균 로짓이 `normal +2.00 / collapse -1.17 /
road_blocked -1.68 / fire -2.68` 로 배경만 양수였다. 데이터 부족에서 온 전형적인
클래스 불균형 붕괴이며, 손실이 내려간 것은 상수 해로 수렴한 결과다.

사람 헤드는 score 0.5 를 넘는 예측이 하나도 없어 confidence 가 전반적으로 눌려 있고,
임계값을 낮춰도 recall 이 0.66 에서 천장을 친다. CenterNet focal loss 에서 양성
픽셀이 극소수일 때 나타나는 현상이다.
