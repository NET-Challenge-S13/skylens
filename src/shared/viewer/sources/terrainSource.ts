...
export const PRESETS: Record<string, Bbox> = {
  ...
  /** 기준 씬이자 데모 비행 구역: 대전 유성 — 충남대 중심, 동쪽으로 카이스트 포함
   *  (~3 km 도심). `skylens_drone`의 기본 웨이포인트와 `CONFIG.geo.anchor`가 모두
   *  이 안에 있다 — 관제탑 지도와 실제 드론이 같은 땅을 가리키게 하려면 이 셋이
   *  함께 움직여야 한다. `src/test/geography.spec.ts`가 이 정합을 강제한다. */
  daejeon: [127.33, 36.355, 127.365, 36.382],
  /** 서울 경복궁 일대 (~2.5 km). 도심 건물 밀도가 높아 표시 옵션 시연에 쓴다. */
  seoul: [126.964, 37.5555, 126.992, 37.5775],
};
...
/**
 * Control-tower bbox. The tower has exactly ONE scene (COMPONENTS.md §4: VWorld
 * real terrain), so an absent `?map=` is not "no terrain" here — it's the
 * default operating area. Never returns null.
 *
 * The default comes from CONFIG.control.defaultMap so the tower's map and the
 * demo fleet's GPS can be kept pointing at the same place from one line. A drone
 * flying 150 km outside the loaded bbox is not a rendering bug but it looks
 * exactly like one, so this is worth having a single owner.
 */
export function controlMapSpec(): Bbox {
  return resolveMapSpec() ?? PRESETS[CONFIG.control.defaultMap] ?? PRESETS[DEFAULT_PRESET];
}
...
/**
 * VWorld serves tilematrix 6..19 ONLY. Below 6 it answers HTTP 200 with an XML
 * ExceptionReport ("tilematrix 파라미터의 값이 유효한 범위를 넘었습니다"), which is
 * why every imagery request must be judged by content-type rather than status
 * (measured 2026-08-19 against the live key).
 */
...
/** Simultaneous tile requests. VWorld silently drops a large concurrent burst
 *  (measured: ~30% of a 64-tile mosaic survived an all-at-once fetch), so the
 *  mosaic is filled through a small pool instead. */
const SAT_CONCURRENCY = 6;
/** Attempts per tile before giving up and leaving a hole. */
const SAT_TILE_ATTEMPTS = 3;
...
/**
 * VWorld needs an API key injected by the dev proxy (vite.config.ts). With no
 * key EVERY imagery tile fails, so a whole-bbox fetch would fire ~160 doomed
 * requests before degrading. Probe ONE tile first and cache the verdict — the
 * keyless path then costs a single request and the caller falls back instantly.
 *
 * The probe tile has to be one VWorld will actually serve, so it is taken from
 * the CALLER'S OWN bbox at SAT_MIN_ZOOM. (The previous z1 probe was outside the
 * served tilematrix range and therefore reported "unreachable" even with a
 * perfectly good key, silently disabling the aerial view for every scene.)
 * z6 tiles are ~600 km wide, so all Korean presets share one cached verdict.
 */
const imageryProbes = new Map<string, Promise<boolean>>();
export function imageryReachable(bbox: Bbox): Promise<boolean> {
  const tx = Math.floor(lonToTileX((bbox[0] + bbox[2]) / 2, SAT_MIN_ZOOM));
  const ty = Math.floor(latToTileY((bbox[1] + bbox[3]) / 2, SAT_MIN_ZOOM));
  const key = `${ty}/${tx}`;
  let probe = imageryProbes.get(key);
  if (!probe) {
    probe = fetch(SAT_URL(SAT_MIN_ZOOM, ty, tx))
      .then((res) => res.ok && (res.headers.get('content-type') ?? '').includes('image'))
      .catch(() => false);
    imageryProbes.set(key, probe);
  }
  return probe;
}
...
async function buildImageryGrid(
  ...
  if (!(await imageryReachable(bbox))) return null;
  // Deepest zoom within budget, floored at SAT_MIN_ZOOM — a shallower one is
  // not served at all, so a huge bbox gets more tiles rather than no imagery.
  let zoom = SAT_MIN_ZOOM;
  ...
  //
  // NOTE: the texture is built with createImageBitmap(ImageData), NOT with
  // canvas.transferToImageBitmap(). The zero-copy transfer throws
  // "ImageBitmap construction failed" under software rendering (headless
  // Chromium, CI) AND it detaches the canvas on the way out — so a fallback
  // that re-read the canvas afterwards got a fully transparent buffer and
  // painted the whole operating area black. Reading pixels first and building
  // the bitmap from them costs one copy and cannot fail that way.
  const canvas = new OffscreenCanvas(width, height);
  const ctx = canvas.getContext('2d');
  if (!ctx) return null;

  // Opaque base coat BEFORE any tile lands. An undrawn region would otherwise
  // keep alpha 0 and rgb 0, and an opaque material samples that as PURE BLACK —
  // one dropped tile then reads as a void punched through the city. A neutral
  // ground tone degrades to "imagery missing here" instead of "hole in world".
  ctx.fillStyle = '#4b5560';
  ctx.fillRect(0, 0, width, height);

  const queue: Array<[number, number]> = [];
  for (let ty = ty0; ty <= ty1; ty++) {
    for (let tx = tx0; tx <= tx1; tx++) queue.push([ty, tx]);
  }

  const drawTile = async (ty: number, tx: number): Promise<boolean> => {
    const res = await fetch(SAT_URL(zoom, ty, tx));
    if (!res.ok) throw new Error(`sat tile HTTP ${res.status}`);
    const type = res.headers.get('content-type') ?? '';
    // VWorld answers errors with HTTP 200 + an XML ExceptionReport, so status
    // alone does not mean we got an image. Decoding XML as a bitmap would throw
    // anyway, but checking here gives a usable message instead of a DOM error.
    if (!type.includes('image')) throw new Error(`sat tile not an image (${type})`);
    const img = await createImageBitmap(await res.blob());
    ctx.drawImage(img, (tx - tx0) * 256, (ty - ty0) * 256);
    return true;
  };

  // Bounded concurrency + one retry. Firing a whole mosaic at once (64 tiles
  // for a city bbox) makes VWorld drop most of them, and the failures were
  // swallowed — the drape came back ~30% painted with the rest left
  // transparent, which renders as a black hole exactly over the operating area.
  let ok = 0;
  let failed = 0;
  let next = 0;
  const worker = async (): Promise<void> => {
    for (;;) {
      ...
      const [ty, tx] = queue[i];
      // A throttled tile usually succeeds on a later attempt, so back off and
      // retry rather than leaving a transparent hole in the operating area.
      let lastErr: unknown = null;
      for (let attempt = 0; attempt < SAT_TILE_ATTEMPTS; attempt++) {
        if (attempt > 0) await new Promise((r) => setTimeout(r, 200 * attempt));
        try {
          await drawTile(ty, tx);
          lastErr = null;
          break;
        } catch (err) {
          lastErr = err;
      ...
      if (lastErr === null) {
        ...
        failed++;
        if (failed <= 3) console.warn(`[terrain] sat tile ${zoom}/${ty}/${tx} failed:`, lastErr);
      }
    }
  };
  await Promise.all(
    Array.from({ length: Math.min(SAT_CONCURRENCY, queue.length) }, () => worker()),
  );
  if (ok === 0) {
    console.warn(`[terrain] no satellite tile decoded at z${zoom} (${cols}x${rows})`);
    return null;
  }
  console.info(
    `[terrain] mosaic z${zoom} ${cols}x${rows}: ${ok} ok, ${failed} failed of ${queue.length}`,
  );

  // Read pixels ONCE, up front. Everything downstream is derived from this
...
/**
 * The satellite mosaic and how to address it from lon/lat. Other layers use it
 * to drape the SAME imagery onto their own geometry — that is what makes the
 * control tower's 실사 항공뷰 option (COMPONENTS.md §4) a single continuous
 * aerial image over terrain AND buildings rather than two unrelated textures.
 */
export interface ImageryProjection {
  texture: ImageBitmap;
  zoom: number;
  /** Mosaic UV (0..1, flipY=false convention) at a geographic point. */
  uvAt(lon: number, lat: number): [number, number];
}

function makeImageryProjection(im: ImageryGrid): ImageryProjection {
  return {
    texture: im.bitmap,
    zoom: im.zoom,
    uvAt(lon: number, lat: number): [number, number] {
      const sx = lonToTileX(lon, im.zoom) * 256 - im.originX;
      const sy = latToTileY(lat, im.zoom) * 256 - im.originY;
      return [
        THREE.MathUtils.clamp(sx / im.width, 0, 1),
        THREE.MathUtils.clamp(sy / im.height, 0, 1),
      ];
    },
  };
}
...
export interface TerrainScene {
  ...
  /** Present with `visual`: lets the building layer share the same drape. */
...
export interface TerrainPatch {
  ...
  /** Cell-local drape, so streamed buildings can share this cell's imagery. */
  imagery?: ImageryProjection;
}
...
export async function loadTerrainPatch(
  ...
  return {
    visual,
    elevCtx: makeElevationCtx(grid, cellBbox, t),
    imagery: imagery ? makeImageryProjection(imagery) : undefined,
  };
}
...
export async function loadTerrainScene(
  ...
  const imageryPromise: Promise<ImageryGrid | null> = wantImagery
    ? buildImageryGrid(bbox).catch((err) => {
        console.warn('[terrain] imagery mosaic threw:', err);
        return null;
      })
