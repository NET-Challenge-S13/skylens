...
import { controlMapSpec, loadTerrainScene, resolveMapSpec } from './terrainSource.ts';
...
// ---------------------------------------------------------------------------
// Control tower scene (COMPONENTS.md §4)
// ---------------------------------------------------------------------------
//
// The tower has exactly ONE scene: VWorld real terrain + real building data. It
// does NOT derive anything from a Gaussian splat — that path above stays for the
// situation board, which renders the splat itself.
//
// One load feeds all three display options (점 / 검정 텍스처 건물 / 실사 항공뷰):
// the point sampling, the prisms, and the aerial UVs all come out of the same
// fetch, so switching option at runtime is a material swap, never a reload.

/** Buildings carry the whole point budget here — the terrain renders as a mesh. */
const CONTROL_BUILDING_POINTS = 60_000;

export interface ControlScene {
  /** Building points, for the 점 display option. */
  data: SceneData;
  /** Textured DEM surface. Null when imagery is unreachable (see `imagery`). */
  terrainVisual: TerrainVisual | null;
  /** Low-res backdrop terrain ring so the world doesn't end at the bbox edge. */
  surroundVisual: TerrainVisual | null;
  /** Extruded prisms + edges + aerial UVs. Null when no footprints were found. */
  buildingVisual: BuildingVisual | null;
  /** Geo↔world transform for this scene — the tower's coordinate boundary. */
  ctx: TerrainContext;
  /** The operating area actually loaded. */
  bbox: Bbox;
  /** Where the footprints came from; the settings panel states this. */
  buildingSource: BuildingSourceKind;
  footprints: number;
  /** False when VWorld imagery is unreachable — 실사 항공뷰 is then unavailable. */
  imageryAvailable: boolean;
  /** Seed for the world streamer (surrounding cells load as the drone travels). */
  streamSeed: { coreBbox: Bbox; ctx: TerrainContext };
}

export async function loadControlScene(
  onProgress?: (percent: number) => void,
): Promise<ControlScene> {
  const bbox = controlMapSpec();
  const terrain = await loadTerrainScene(bbox, onProgress, 0);

  // Best-effort building layer: VWorld is proxy-only (no CORS), so a missing
  // proxy/key or an API hiccup must degrade to terrain-only rather than break
  // the operator's screen mid-demo.
  let bld: BuildingLayer | null = null;
  try {
    bld = await loadBuildings(bbox, terrain.ctx, CONTROL_BUILDING_POINTS, {
      imagery: terrain.imagery,
      allowSynthetic: true,
    });
  } catch (err) {
    console.warn('[control] building layer unavailable — terrain-only scene:', err);
  }

  // Terrain is sampled with a ZERO point budget above: the ground renders as a
  // textured mesh, so every point in the tower's cloud is a BUILDING point and
  // the 점 option shows exactly what COMPONENTS.md §4 says it shows.
  const data: SceneData =
    bld && bld.points.count > 0
      ? {
          positions: bld.points.positions,
          colors: bld.points.colors,
          count: bld.points.count,
          bounds: bld.points.bounds,
          groundY: 0,
        }
      : {
          ...
          count: 0,
          // A zero-point cloud leaves an EMPTY Box3 (min +inf / max -inf), which
          // turns any later span arithmetic into NaN. Fall back to the scene's
          // own normalized footprint instead.
          bounds: new THREE.Box3(
            new THREE.Vector3(-TARGET_EXTENT / 2, 0, -TARGET_EXTENT / 2),
            new THREE.Vector3(TARGET_EXTENT / 2, TARGET_EXTENT / 4, TARGET_EXTENT / 2),
          ),
          groundY: 0,
        };
  ...
  return {
    data,
    terrainVisual: terrain.visual ?? null,
    surroundVisual: terrain.surround ?? null,
    buildingVisual: bld?.visual ?? null,
    ctx: terrain.ctx,
    bbox,
    buildingSource: bld?.source ?? 'none',
    footprints: bld?.footprints ?? 0,
    imageryAvailable: terrain.imagery != null,
    streamSeed: { coreBbox: bbox, ctx: terrain.ctx },
  };
}
