...
def find_road_indices(cache_dir: Path, road_class: int = 3) -> list:
    """리사이즈 캐시에서 road_blocked 픽셀이 있는 항목의 인덱스를 찾는다.

    3,595장을 매 실행마다 훑을 이유가 없으므로 결과를 캐시 디렉터리에 남기고
    다음부터는 그것을 읽는다.
    """
    import numpy as np

    from skylens_model.utils import ResizedCache

    marker = cache_dir / f"has_class_{road_class}.json"
    if marker.is_file():
        return json.loads(marker.read_text(encoding="utf-8"))

    ds = ResizedCache(cache_dir, transforms=None)
    found = []
    for i in range(len(ds)):
        mask = ds[i].get("danger_mask")
        if mask is not None and bool(np.any(np.asarray(mask) == road_class)):
            found.append(i)
        if (i + 1) % 500 == 0:
            print(f"    {i + 1:,}/{len(ds):,}장 확인, 지금까지 {len(found):,}장")
    marker.write_text(json.dumps(found), encoding="utf-8")
    print(f"  road_blocked 포함 {len(found):,}/{len(ds):,}장 ({100 * len(found) / max(len(ds), 1):.1f}%)")
    return found
        ...
        for cls, root, tr_split, ev_split in sources:
            ...
            try:
                ...
                if (
                    ...
                    and cls is RescueNetSegmentation
                ):
                    idx = find_road_indices(cache_dir)
                    if idx:
                        # 원본 1회 + 해당 이미지 (N-1)회 추가 = N배
